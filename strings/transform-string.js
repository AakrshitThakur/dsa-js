...
function transform(A, B) {
  ...
  for (let ch of A) freqA[ch] = (freqA[ch] || 0) + 1;
  for (let ch of B) freqB[ch] = (freqB[ch] || 0) + 1;
  for (let key in freqA) {
    if (freqA[key] !== freqB[key]) return -1;
  }
...
console.log(transform("abd", "bad"));
console.log(transform("GeeksForGeeks", "ForGeeksGeeks"));